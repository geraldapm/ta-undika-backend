...
const String BEACON_UUID = "88973355-ff01-42b9-cfed-9d0ff2b3219d"; 
...
//define sensors
#define DHTSensor 32
#define LM35Sensor 34

DHT dht1(DHTSensor,11);
...
bool RegisterSensor() {
  ...
  doc["sensors"][0]["type"] = "DHT11";
  doc["sensors"][0]["value"] = 0;
  doc["sensors"][0]["valueUnit"] = "Humidity";
  doc["sensors"][0]["displayUnit"] = "%";
  doc["sensors"][0]["minValue"] = 0;
  doc["sensors"][0]["maxValue"] = 100;
...
void setBeacon(char* url)
{
  ...
  oScanResponseData.setName("MadeBeacon1");
...
int cnt = 0;
void loop() {
  //sample suhu & kelembaban dengan angka random
  //suhu = random(0, 100);
  //humid = random(0, 100);
  //suhu = (int)hitungSuhu(analogRead(LM35Sensor));
  //suhu = dht1.readTemperature();
  suhu = hitungSuhu(analogRead(LM35Sensor));
  //suhu = analogRead(LM35Sensor);
